...
if [[ $COMMON_LIBRARY == 'Y' ]]; then
  ...
  yum -y install bash-completion bzip2 ca-certificates epel-release gcc gcc-c++ lrzsz make net-tools redhat-lsb tar vim yum-utils \
                 curl-devel gdbm-devel gd-devel glib2-devel glibc-devel ImageMagick-devel libcurl-devel libffi-devel libxml2-devel libxslt-devel mariadb-devel ncurses-devel openssl-devel pcre-devel libpq-devel readline-devel zlib-devel
